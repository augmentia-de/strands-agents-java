...
class EnhancedMultiAgentTest {
    ...
    void a2aExecutorShouldHandleTimeout() {
        var slowAgent = new StrandsAgent(new MockChatModel() {
            @Override
            public dev.langchain4j.model.chat.response.ChatResponse chat(
                    dev.langchain4j.model.chat.request.ChatRequest request) {
    ...
    void a2aExecutorShouldRetryOnFailure() {
        ...
        var failingAgent = new StrandsAgent(new MockChatModel() {
            @Override
            public dev.langchain4j.model.chat.response.ChatResponse chat(
                    dev.langchain4j.model.chat.request.ChatRequest request) {
    ...
    void agentToolShouldWorkWithA2AExecutor() {
        var subAgent = new StrandsAgent(new MockChatModel("Sub: %s"));
        var executor = new A2AExecutor();
    ...
    void swarmOrchestratorShouldUseLlmRouter() {
        var weatherAgent = new StrandsAgent(new MockChatModel("Wetter: %s"));
    ...
    void fullOrchestrationWithA2AExecutor() {
        ...
        var mainAgent = new StrandsAgent(new MockChatModel(), registry, new ToolExecutor());
